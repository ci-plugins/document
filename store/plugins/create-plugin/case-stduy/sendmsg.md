...
![](../../../../.gitbook/assets/image-plugin-casestudy-sendmsg6.png)
...
1. sdk.get\_input()获取组件的输入，输入组件获取到的值都是字符串
...
2. sdk.get\_sensitive\_conf("key")获取私有配置变量
...
4. exit\_with\_succ(data=data)设置输出变量
...
3. 在任意位置新建文件夹，如 sendmsgDemo\_release
4. 将步骤 2 生产的执行包拷贝到 sendmsgDemo\_release 下
5. 添加task.json文件到 sendmsgDemo\_release 下
6. 把 sendmsgDemo\_release 使用`zip -r sendmsgDemo.zip sendmsgDemo_release`打成zip包即可
