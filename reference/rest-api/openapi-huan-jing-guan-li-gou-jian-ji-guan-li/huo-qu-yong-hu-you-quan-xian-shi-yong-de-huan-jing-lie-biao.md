...
| HTTP代码 | 说明 | 参数类型 |
| :--- | :--- | :--- |
| 200 | successful operation | \[数据返回包装模型List环境信息\(权限\)\]\(\#数据返回包装模型List环境信息\(权限\)\) |

#### 请求样例

```javascript
curl -X GET '[请替换为上方API地址栏请求地址]?app_secret={app_secret}&amp;app_code={app_code}'
```
...
| 参数名称 | 参数类型 | 必须 | 参数说明 |
| :--- | :--- | :--- | :--- |
| data | List&lt;\[环境信息\(权限\)\]\(\#环境信息\(权限\)\)&gt; | 否 | 数据 |
